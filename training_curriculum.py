...
def main():
    ...
    # ✱ use the true action count, not hard‑coded “8”
    n_actions = len(COMPLEX_MOVEMENT)

    policy_net = DuelingDQN(4, n_actions, device).to(device)
    target_net = DuelingDQN(4, n_actions, device).to(device)
    sync_networks(policy_net, target_net)
    optimizer = optim.Adam(policy_net.parameters(), lr=1e-4)

    print("Using device:", device)
    if resume_training and os.path.exists("mario_q_curriculum.pth") and os.path.exists("mario_q_target_curriculum.pth"):
        policy_net.load_state_dict(torch.load("mario_q_curriculum.pth", map_location=device))
        target_net.load_state_dict(torch.load("mario_q_target_curriculum.pth", map_location=device))
    ...
    total_curr       = episodes_per_level * len(levels)
    total_episodes   = total_curr + random_episodes
    per_alpha        = 0.5
    beta_start, beta_end = 0.4, 1
    beta_inc_episodes    = int(0.1 * total_curr)
    ...
    for episode in range(total_episodes):
        if episode < total_curr:
            lvl_idx = episode // episodes_per_level
            lvl     = levels[lvl_idx]
            beta    = beta_start + (beta_end - beta_start) * (episode / beta_inc_episodes) if episode < beta_inc_episodes else beta_end
        else:
            lvl  = random.choice(levels)
            beta = beta_end

        raw_env = gym_super_mario_bros.make(lvl)
        ...
        while not done:
            ...
            if len(replay_buffer) > 500:
                loss = train_step(
                    policy_net, target_net, replay_buffer,
                    batch_sz, discount_factor, optimizer,
                    device, beta
                )
                ...
                if train_counter % target_update_freq == 0:
                    sync_networks(policy_net, target_net)
                    torch.save(policy_net.state_dict(), "mario_q_curriculum.pth")
                    torch.save(target_net.state_dict(), "mario_q_target_curriculum.pth")
